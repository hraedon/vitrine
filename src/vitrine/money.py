...
# The closed registry. Seed: the currencies the world wing's rooms price in.
# USD and GBP share Python's default separators and symbol-before placement,
# so seeding them cannot perturb any existing US output. JPY's minor unit is
# the yen itself (no decimals) — the Japan rooms author amount_minor in whole
# yen (e.g. 65141 → ¥65,141).
CURRENCIES: dict[str, Currency] = {
    "USD": Currency("USD", "$", symbol_before=True, minor_digits=2),
    "GBP": Currency("GBP", "£", symbol_before=True, minor_digits=2),
    "JPY": Currency("JPY", "¥", symbol_before=True, minor_digits=0),
}
